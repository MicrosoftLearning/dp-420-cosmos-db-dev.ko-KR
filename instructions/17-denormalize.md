...
1. 줄 589에서 **{container to update}를** `product`로 바꿉니다. 제품 범주 이름이 업데이트되면 해당 범주의 모든 제품을 새 제품 범주 이름으로 업데이트해야 합니다.
...
1. 줄 603에서는 변경 피드에 처리해야 할 새 변경 내용이 있을 때 호출될 일부 코드를 추가해야 합니다. 이렇게 하려면, 다음 코드 조각을 복사하여 **//To-Do:** 으로 시작하는 줄 아래에 붙여넣습니다.
...
1. 이제 코드가 다음 이미지의 코드와 같이 표시됩니다.
    ...
    기본적으로 변경 피드는 매초 실행됩니다. 관찰된 컨테이너에서 삽입 또는 업데이트가 많이 수행되는 시나리오의 경우, 대리자는 둘 이상의 변경 내용을 포함할 수도 있습니다. 이런 이유로 대리자 **input** 을 **IReadOnlyCollection** 으로 입력합니다.
...
1. 다음 코드 조각을 복사하여 **//To-Do:** 로 시작하는 줄 아래에 붙여넣습니다. 함수는 두 작업을 수행합니다. 먼저 **categoryId** 에서 전달된 모든 제품에 대한 제품 컨테이너를 쿼리합니다. 그런 다음 각 제품을 새 제품 범주 이름으로 업데이트합니다.
    ...
    **foreach()** 루프를 사용하여 쿼리에서 반환되는 각 제품으로 진행합니다. 그런 다음, 각 행에 대해 업데이트된 제품 개수를 알 수 있도록 카운터를 업데이트합니다. 그런 다음, 제품의 범주 이름을 새 **categoryName** 으로 업데이트합니다. 마지막으로, **ReplaceItemAsync()** 를 호출하여 제품 컨테이너에서 제품을 다시 업데이트합니다.
...
- 해당 고객에 대한 새 판매 주문을 만들고 해당 **salesOrderCount** 속성을 업데이트합니다.
...
1. 이제 다음 코드가 표시됩니다.
    ...
    먼저 **ReadItemAsync()** 를 호출하고 **customerId** 를 파티션 키와 ID로 전달하여 고객 레코드를 검색합니다.

1. 줄 483의 **//To-Do:** 주석 아래에서 다음 코드 조각을 붙여넣어 **salesOrderCount** 의 값을 증분시킵니다.
...
1. 몇 줄 아래로 스크롤하여, 고객에 대해 만들 새 판매 주문에 대한 데이터를 확인합니다.
    ...
    판매 주문 헤더에는 **orderId**, **customerId**, **orderDate**, **shipDate** 가 있으며 이 헤더는 비워 둡니다.
...
1. 또 다른 **//To-Do:** 주석까지 약간 더 스크롤합니다. 새 판매 주문을 삽입하고 트랜잭션 일괄 처리를 사용하여 고객 레코드를 업데이트하기 위한 코드를 추가해야 합니다.

1. 다음 코드 조각을 복사한 후 **//To-Do:** 주석 아래 줄에 붙여넣습니다.
    ...
    이 코드는 컨테이너 개체에서 **CreateTransactionalBatch()** 를 호출합니다. 모든 트랜잭션이 단일 논리적 파티션으로 범위가 지정되므로, 파티션 키 값을 필수 매개 변수로 사용합니다. 또한 **CreateItem()** 을 호출하는 새 판매 주문 및 **ReplaceItem()** 을 호출하는 업데이트된 고객 개체를 전달합니다. 그런 다음, **ExecuteAsync()** 를 호출하여 트랜잭션을 실행합니다.
...
**customerId** 를 파티션 키로 사용하여 고객 및 모든 판매 주문을 동일한 컨테이너에 저장하도록 데이터베이스를 디자인했으므로, 고객 컨테이너를 쿼리하고 고객의 레코드와 고객의 모든 판매 주문을 단일 작업으로 반환할 수 있습니다.
...
1. **고객 레코드 및 모든 주문 출력** 까지 위로 스크롤합니다.
   ...
   **salesOrderCount** 속성은 두 개의 판매 주문을 표시합니다.
...
1. 다시 **고객 레코드와 모든 주문 출력** 까지 위로 스크롤합니다.
   ...
   **salesOrderCount** 속성은 세 개의 판매 주문을 표시합니다.
...
1. Ctrl + G를 선택한 다음, **529** 를 입력합니다.
    ...
    여기에서 코드가 먼저 고객 레코드를 검색한 다음, **salesOrderCount** 가 1씩 감소하는 것을 볼 수 있습니다.

    다음으로, **CreateTransactionalBatch()** 를 호출합니다. 다시 논리 파티션 키 값이 전달되지만, 이번에는 **DeleteItem()** 이 주문 ID로 호출되고 **ReplaceItem()** 이 업데이트된 고객 레코드로 호출됩니다.
...
1. Ctrl + G를 선택한 다음, **566** 을 입력합니다.
    ...
    매우 간단한 이 쿼리는 반환되는 레코드 수를 제한하는 **TOP** 문과 내림차순으로 **salesOrderCount** 속성에 대한 **ORDER BY** 를 사용합니다.
